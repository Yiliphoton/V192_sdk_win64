...
/**
 * Install a point-cloud filter into the decoder pipeline.
 *
 * @param filter_cb   Function called with (points array, count, is_dual, user_data).
 * @param user_data   Passed through to filter_cb unchanged.
 *
 * Pass NULL for filter_cb to remove any installed filter.
 */
typedef void (*V192FilterCallback)(V192Point* points, uint32_t* count, int is_dual, void* user_data);
V192_API void v192_set_filter(V192DecoderHandle h, V192FilterCallback filter_cb, void* user_data);
