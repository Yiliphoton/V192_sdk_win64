...
V192 SDK 提供 `v192_filter.dll`，用于解码速腾聚创 V192 192 通道激光雷达（MSOP 协议）的点云数据，并内置近邻滤波器。支持实时 UDP 接收、PCAP 离线回放和手动数据包注入三种模式。
...
The V192 SDK ships `v192_filter.dll`, a Windows x64 library for decoding point-cloud data from the RoboSense V192 192-channel LiDAR (MSOP protocol). It bundles the decoder and a neighbour-based point-cloud filter in a single DLL.
